...
def _agent_patches(overrides: dict):
    base = {
        ...
        "src.crawling.agent.find_element_id": AsyncMock(return_value="3"),
...
async def test_crawl_text_extraction_succeeds():
    ...
    with ExitStack() as stack:
        ...
        value, screenshot, raw = await crawl("https://example.com", "BTC price", "line")
...
async def test_crawl_image_fallback_when_text_parse_fails():
    ...
    with ExitStack() as stack:
        for target, mock in patches.items():
            stack.enter_context(patch(target, mock))
        value, screenshot, _ = await crawl("https://example.com", "BTC price", "line")
...
async def test_crawl_scrolls_and_retries():
    ...
    async def find_after_scroll(page, query, **kwargs):
        nonlocal call_count
        call_count += 1
        return "5" if call_count >= 2 else None
    ...
    with ExitStack() as stack:
        for target, mock in patches.items():
            stack.enter_context(patch(target, mock))
        value, _, _ = await crawl("https://example.com", "price", "line")
...
async def test_crawl_returns_none_after_all_attempts_fail():
    ...
    with ExitStack() as stack:
        for target, mock in patches.items():
            stack.enter_context(patch(target, mock))
        value, screenshot, msg = await crawl("https://example.com", "BTC price", "line")
...
async def test_crawl_flag_true():
    ...
    with ExitStack() as stack:
        for target, mock in patches.items():
            stack.enter_context(patch(target, mock))
        value, _, _ = await crawl("https://example.com", "Is site online?", "flag")
